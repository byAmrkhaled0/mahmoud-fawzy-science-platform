(function(){
  ...
  function initAdminUpgrades(){
    ...
    if(!adminSections.some(section=>section[0]==='errors')) adminSections.splice(Math.max(0,adminSections.length-2),0,['errors','database','أخطاء الموقع']);
    ...
    window.toggleSchedule=function(id){const item=adminData.groups.find(group=>String(group.id)===String(id));if(!item)return;item.active=item.active===false;persist(item.active?'تم تفعيل الموعد':'تم إيقاف الموعد مؤقتًا');renderSchedules();};
    window.deleteSchedule=async function(id){if(!isTeacherManager())return aToast('الحذف متاح للمدرس أو المدير فقط');const item=adminData.groups.find(group=>String(group.id)===String(id));if(!item||!confirm(`حذف موعد ${item.name||''}؟ سيختفي من نموذج الحجز فورًا.`))return;try{await window.MFCloud?.deleteDocument?.('groups',id);adminData.groups=adminData.groups.filter(group=>String(group.id)!==String(id));saveData(adminData);aToast('تم حذف الموعد من الموقع ونموذج الحجز');renderSchedules();}catch(error){aToast('تعذر حذف الموعد. تحقق من الإنترنت وحاول مرة أخرى.');}};
    ...
    window.editStudent=function(code){
      ...
      s.name=s.studentName=name;s.parentPhone=parentPhone;s.grade=grade;s.group=group;s.academicYear=academicYear;s.term=ACADEMIC_TERMS.includes(term)?term:currentAcademicContext().term;s.notes=prompt('ملاحظات المدرس',s.notes||'')??s.notes??'';
      persist('تم تحديث بيانات الطالب');renderStudents();
    };
    window.deleteStudent=async function(code){
      ...
      try{await window.MFCloud?.deleteStudentSafely?.(student||{studentCode:code});adminData.students=adminData.students.filter(s=>stCode(s)!==code);saveData(adminData);aToast('تم حفظ نسخة استرجاع وحذف الطالب');renderStudents();}
      catch(error){aToast('تعذر الحذف الآمن. لم يتم حذف الطالب من الواجهة.');}
    };
    ...
    renderSettings=function(){
      ...
      document.getElementById('settingsForm').onsubmit=e=>{e.preventDefault();const values=Object.fromEntries(new FormData(e.target).entries());if(!ACADEMIC_YEAR_PATTERN.test(values.academicYear))return aToast('صيغة العام الدراسي غير صحيحة');adminData.settings={...adminData.settings,...values};persist('تم حفظ إعدادات العام الدراسي والمنصة');renderSettings();};
    };
    ...
    window.deleteItem=function(collection,id){if(!isTeacherManager())return aToast('الحذف متاح للمدرس أو المدير فقط');deleteItemV52(collection,id);};
    const forceSyncV52=window.forceFirestoreSync;
    ...
    window.setPaid=async function(code,value){setPaidV53(code,value);const student=adminData.students.find(item=>stCode(item)===code);try{if(student)await window.MFCloud?.saveStudent?.(student);aToast('تم تحديث حالة الدفع');}catch(error){aToast('تم التعديل على الجهاز، لكن تعذر تحديث الموقع.');}};
  }
  ...
  const studentHelpItems=[
    ...
    {q:'ولي الأمر يتابع إزاي؟',keys:'ولي الامر الاب الام متابعة تقرير',a:'يستخدم ولي الأمر الكود المنفصل المخصص له لعرض الحضور والدرجات والواجبات والملاحظات.',link:'parent.html',label:'فتح صفحة ولي الأمر'},
