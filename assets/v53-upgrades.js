(function(){
  ...
  if(typeof publicDataOnly==='function'){
    publicDataOnly=function(data){
      ...
      return {...defaultData(),materials:d.materials,questions:d.questions,reviews:d.reviews.filter(r=>r.approved===true),groups:d.groups,assignments:d.assignments,settings:{siteUrl:settings.siteUrl||DEFAULT_SITE_URL,teacherPhone:settings.teacherPhone||TEACHER_WHATSAPP,teacherName:settings.teacherName||'مستر محمود إبراهيم فوزي',homeNotice:settings.homeNotice||'',academicYear:settings.academicYear||calculatedAcademicYear(),term:settings.term||'الترم الأول'}};
    };
  }
  ...
  function initAdminUpgrades(){
    ...
    window.refreshStudentsTable=function(){
      let rows=adminData.students.map(student=>typeof academicStudent==='function'?academicStudent(student):normalizeStudent(student));
      const q=normalizeText(document.getElementById('studentSearchAdmin')?.value||'');
      ...
      if(q)rows=rows.filter(s=>normalizeText([s.studentCode,s.parentCode,s.name,s.parentPhone,s.studentPhone,s.group].join(' ')).includes(q));
      if(grade!=='all')rows=rows.filter(s=>sameAcademicValue(s.grade,grade));
      if(pay==='paid')rows=rows.filter(s=>s.paid);if(pay==='unpaid')rows=rows.filter(s=>!s.paid);
      if(year!=='all')rows=rows.filter(s=>sameAcademicValue(s.academicYear||currentAcademicContext().academicYear,year));
      if(term!=='all')rows=rows.filter(s=>sameAcademicValue(s.term||currentAcademicContext().term,term));
    ...
    renderExams=function(){
      ...
      if(typeof ensureExamScoreControls==='function')ensureExamScoreControls(form);
      if(form&&!form.querySelector('[name="openAt"]')){
        ...
        form.addEventListener('submit',event=>{const open=form.openAt.value,close=form.closeAt.value;if(open&&close&&examTimeMs(close)<=examTimeMs(open)){event.preventDefault();event.stopImmediatePropagation();aToast('موعد إغلاق الامتحان يجب أن يكون بعد موعد الفتح');}},true);
      }
      ...
      if(form){
        ...
        form.onsubmit=async event=>{
          ...
          if(ex.openAt&&ex.closeAt&&examTimeMs(ex.closeAt)<=examTimeMs(ex.openAt))return aToast('موعد إغلاق الامتحان يجب أن يكون بعد موعد الفتح');
          ...
          try{
            ...
            ex.id=`ex-${Date.now()}`;ex.openAt=examCairoIso(ex.openAt);ex.closeAt=examCairoIso(ex.closeAt);ex.allowRetake=!!ex.allowRetake;ex.active=true;ex.questionCount=parsed.length;ex.mcqCount=parsed.filter(q=>q.type==='mcq').length;ex.essayCount=parsed.filter(q=>q.type==='essay').length;
      ...
      if(existing&&!document.getElementById('examScheduleSummary')){
        existing.insertAdjacentHTML('beforeend',`<div id="examScheduleSummary" class="exam-schedule-summary"><h3>الامتحانات المنشورة والوقت المتبقي</h3>${adminData.exams.map(ex=>{const state=examClientAvailability(ex,Date.now()),open=examTimeMs(ex.openAt)||0,close=examTimeMs(ex.closeAt)||0,target=state==='upcoming'?open:state==='open'?close:0,label=state==='upcoming'?'يفتح بعد':state==='open'?(close?'يغلق بعد':'متاح الآن'):'انتهى الوقت';return `<div class="mobile-row admin-live-exam" data-admin-exam-target="${target}" data-admin-exam-kind="${state==='upcoming'?'opening':state==='open'?'closing':'closed'}"><div><b>${safe(ex.title)}</b><small>${safe(ex.grade||'كل الصفوف')} · ${safe(ex.group||'كل المجموعات')} · ${safe(ex.academicYear||ctx.academicYear)} · ${safe(ex.term||ctx.term)}</small><small>${ex.openAt?`يفتح: ${safe(formatExamCairoDateTime(ex.openAt))}`:'متاح فورًا'}${ex.closeAt?` · يغلق: ${safe(formatExamCairoDateTime(ex.closeAt))}`:''}</small></div><span class="badge ${examAvailabilityClass(ex)}">${safe(examAvailabilityText(ex))}</span><b class="admin-exam-countdown"><small>${label}</small> <span data-admin-countdown>${target?'--:--:--':state==='open'?'بدون موعد إغلاق':'انتهى'}</span></b>${ex.pdfUrl?`<a class="small-btn" href="${safe(ex.pdfUrl)}" target="_blank" rel="noopener noreferrer">فتح PDF</a>`:''}</div>`;}).join('')||'<p class="section-desc">لا توجد امتحانات.</p>'}</div>`);
        const tickAdminExamCountdowns=()=>document.querySelectorAll('[data-admin-exam-target]').forEach(row=>{const target=Number(row.dataset.adminExamTarget||0),out=row.querySelector('[data-admin-countdown]');if(!out||!target||row.dataset.countdownExpired==='1')return;const left=Math.max(0,target-Date.now()),total=Math.ceil(left/1000),days=Math.floor(total/86400),hours=Math.floor(total%86400/3600),minutes=Math.floor(total%3600/60),seconds=total%60;out.textContent=days?`${days} يوم ${hours} ساعة`:`${String(hours).padStart(2,'0')}:${String(minutes).padStart(2,'0')}:${String(seconds).padStart(2,'0')}`;if(!left){row.dataset.countdownExpired='1';const opening=row.dataset.adminExamKind==='opening',badge=row.querySelector('.badge'),label=row.querySelector('.admin-exam-countdown small');out.textContent=opening?'متاح الآن':'انتهى';if(label)label.textContent=opening?'متاح':'مغلق';if(badge){badge.textContent=opening?'متاح':'مغلق';badge.className=`badge ${opening?'good':'danger'}`;}}});
    ...
    function examAvailabilityText(ex){const state=examClientAvailability(ex,Date.now());return state==='open'?'متاح الآن':state==='upcoming'?'لم يبدأ بعد':'انتهى الوقت';}
    function examAvailabilityClass(ex){return examAvailabilityText(ex)==='متاح الآن'?'good':examAvailabilityText(ex)==='لم يبدأ بعد'?'warn':'danger';}
    window.exportGradesCSV=function(){const rows=(adminData.examAttempts||[]).map(a=>[a.studentCode,a.studentName,a.examTitle,a.score??'',a.maxScore??100,a.percentage??'',a.autoScore??'',a.status,a.submittedAt,a.academicYear||'',a.term||'']);downloadCSV('exam-grades-v54.csv',['كود الطالب','اسم الطالب','الامتحان','درجة الطالب','الدرجة النهائية','النسبة المئوية','الدرجة التلقائية','الحالة','تاريخ التسليم','العام الدراسي','الترم'],rows);};
