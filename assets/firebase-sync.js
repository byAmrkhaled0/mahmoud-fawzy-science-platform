  ...
  try{
    ...
    window.MFCloud = {
      ...
      createStudentAccess: async student => {
        if(!calls.createStudentAccess) throw new Error('Secure student creation function is unavailable');
        return calls.createStudentAccess(student);
      },
      createBooking: async booking => {
        if(cfg.useSecureFunctions !== false && calls.createBooking) return calls.createBooking(booking);
        throw new Error('Secure booking function is not available');
      },
      getBookingStatus: async code => {
        if(!calls.getBookingStatus) throw new Error('Secure booking status function is unavailable');
        return calls.getBookingStatus({code:normalizeCode(code)});
      },
      saveReview: async review => {
        if(cfg.useSecureFunctions !== false && calls.createReview) return calls.createReview(review);
        throw new Error('Secure review function is not available');
      },
