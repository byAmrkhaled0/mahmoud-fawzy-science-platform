...
function persist(msg,meta){ensureCollections(); saveData(adminData); if(window.MFCloud?.saveSiteData) window.MFCloud.saveSiteData(adminData).catch(()=>aToast('تم الحفظ محليًا، وتعذرت مزامنة Firebase')); if(msg){aToast(msg);window.MFCloud?.logActivity?.(msg,meta||{}).catch(()=>{});}}
...
function normalizeStudent(st){const code=String(stCode(st)||'').toUpperCase(); return {...st,id:code,code,studentCode:code,parentCode:String(st.parentCode||'').toUpperCase(),name:stName(st),studentName:stName(st),active:st.active!==false};}
...
window.deleteStudent=async function(code){if(!confirm('حذف الطالب وكل بياناته؟'))return; const student=adminData.students.find(s=>stCode(s)===code); adminData.students=adminData.students.filter(s=>stCode(s)!==code); persist('تم حذف الطالب',{studentCode:code}); try{await window.MFCloud?.deleteStudentPortals?.(student||{studentCode:code});}catch(e){} renderStudents();};
...
window.regenerateParentCode=async function(code){
  ...
  if(!confirm('إنشاء كود ولي أمر جديد؟ سيتوقف الكود السابق فور نجاح مزامنة Firebase.'))return;
  ...
  }catch(e){aToast('تم تحديث الكود محليًا، لكن راجع اتصال Firebase لإيقاف الكود السابق.');}
  renderStudents();
};
window.regenerateStudentCode=async function(code){
  const s=adminData.students.find(x=>stCode(x)===code);if(!s)return;
  if(!confirm('سيتم تغيير كود الطالب وتحديث سجلاته في Firebase. يجب إرسال الكود الجديد للطالب. متابعة؟'))return;
  ...
  try{await window.MFCloud?.migrateStudentCode?.(oldCode,newCode,s);}catch(e){aToast('تم تغيير الكود محليًا لكن ترحيل بعض سجلات Firebase يحتاج مراجعة');}
  renderStudents();
};
...
window.approveBooking=async function(code){
  const b=adminData.bookings.find(x=>(x.code||x.id)===code);if(!b)return;
  let student=adminData.students.find(s=>String(s.bookingCode||'')===String(code));
  if(!student){
    const studentCode=b.studentCode&&String(b.studentCode).startsWith('ST-')?String(b.studentCode).toUpperCase():newStudentCode();
    const parentCode=b.parentCode&&String(b.parentCode).startsWith('PR-')?String(b.parentCode).toUpperCase():newParentCode();
    student={...b,bookingCode:code,code:studentCode,studentCode,parentCode,studentName:b.studentName||b.name,name:b.name||b.studentName,paid:false,active:true,attendance:[],grades:[],homeworks:[],recitations:[]};
    adminData.students.push(student);
  }
  b.studentCode=student.studentCode;b.parentCode=student.parentCode;b.studentName=b.studentName||b.name;b.status='تم القبول وتحويله لطالب';
  persist('تم قبول الحجز وإنشاء أكواد الدخول',{bookingCode:code,studentCode:student.studentCode});
  try{await window.MFCloud?.saveStudent?.(student);}catch(err){aToast('تم الحفظ محليًا وتعذرت مزامنة بيانات الطالب');}
  renderBookings();
};
window.deleteBooking=async function(code){if(!confirm('حذف الحجز؟'))return; adminData.bookings=adminData.bookings.filter(b=>b.code!==code); persist('تم حذف الحجز'); await Promise.all([cloudDelete('bookings',code),cloudDelete('booking_status',code)]); renderBookings();};
...
function renderSettings(){fresh(); content(`<div class="section-head"><div><span class="kicker"><span data-icon="sparkles"></span> الإعدادات</span><h2 class="section-title">إعدادات الموقع والدومين</h2></div></div><div class="card"><form id="settingsForm" class="grid"><input name="siteUrl" value="${safe(adminData.settings.siteUrl||DEFAULT_SITE_URL||'')}" placeholder="الدومين الأساسي"><input name="teacherPhone" value="${safe(adminData.settings.teacherPhone||TEACHER_WHATSAPP||'')}" placeholder="رقم واتساب المدرس"><textarea name="homeNotice" placeholder="رسالة تنبيه للطلاب">${safe(adminData.settings.homeNotice||'')}</textarea><button class="btn primary"><span data-icon="sparkles"></span> حفظ الإعدادات</button></form></div><div class="grid grid-2" style="margin-top:18px"><div class="seo-card"><h3>SEO</h3><p>تم جعل صفحة المدرس noindex، ولا يوجد رابط ظاهر للوحة المدرس في الموقع العام.</p></div><div class="seo-card"><h3>أمان Firebase</h3><p>انشر Cloud Functions ثم Firestore وStorage Rules وIndexes من ملفات المشروع.</p></div></div>`); document.getElementById('settingsForm').onsubmit=e=>{e.preventDefault(); adminData.settings={...adminData.settings,...Object.fromEntries(new FormData(e.target).entries())}; persist('تم حفظ الإعدادات');};}

function renderSection(){({overview:renderOverview,students:renderStudents,bookings:renderBookings,attendance:renderAttendance,payments:renderPayments,exams:renderExams,materials:renderMaterials,reviews:renderReviewsAdmin,backup:renderBackup,settings:renderSettings}[currentSection]||renderOverview)();}
