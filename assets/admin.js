...
window.quickAdminCreate=function(type){
  ...
  const academicKind=['assignment','exam'].includes(type)?type:'';
  const target={student:'students',schedule:'schedules',assignment:'academics',exam:'academics',attendance:'attendance'}[type];if(!target)return;goAdminSection(target);
  setTimeout(()=>{if(academicKind){const trigger=document.querySelector(`[data-open-composer="${academicKind}"]`);trigger?.click();return;}const focusTarget={student:'#addStudentForm [name="name"]',schedule:'#scheduleForm [name="name"]'}[type];if(type==='attendance')return openQrScanner();const field=document.querySelector(focusTarget||'');field?.focus();field?.scrollIntoView({behavior:'smooth',block:'center'});},90);
};
...
function scoreText(row){
  ...
  return `${score} من ${Number(max)||100}`;
}
...
function examAttemptScoreText(a){const max=Number(a.maxScore||100),autoMax=Number(a.autoMaxScore||max);return (a.score!==null&&a.score!==undefined&&a.score!=='')?`${a.score} من ${max}`:(a.autoScore!==null&&a.autoScore!==undefined?`تلقائي ${a.autoScore} من ${autoMax} · ينتظر النهائي من ${max}`:'بانتظار التصحيح');}
...
window.correctAttempt=function(id){const a=adminData.examAttempts.find(x=>String(x.id)===String(id));if(!a)return;document.querySelector('.correction-modal-v40')?.remove();const answers=Array.isArray(a.answers)?a.answers:[],max=Number(a.maxScore||answers.reduce((sum,ans)=>sum+(Number(ans.points)||1),0)||100);const html=`<div class="correction-modal-v40"><div class="correction-card-v40 card"><div class="profile-top"><div><span class="kicker">تصحيح امتحان من ${safe(max)}</span><h2>${safe(a.studentName||'-')}</h2><p class="section-desc">${safe(a.examTitle||'امتحان')} · ${safe(a.studentCode||'-')}</p></div><button class="small-btn danger" onclick="closeCorrectionModal()">إغلاق</button></div><div class="correction-list-v40">${answers.map((ans,i)=>{const points=Number(ans.points||1),awarded=ans.awardedScore===null||ans.awardedScore===undefined?(ans.correct===true?points:0):Number(ans.awardedScore);return `<div class="correction-question-v40"><h3>${i+1}. ${safe(ans.question||'سؤال')} <span class="badge">${safe(points)} درجة</span></h3><div class="correction-answer-grid-v40"><div><span>إجابة الطالب</span><p>${safe(ans.answer||'-')}</p></div><div><span>الإجابة الصحيحة / النموذجية</span><p>${safe(ans.correctAnswer||ans.modelAnswer||'يصححها المدرس')}</p></div></div><label class="correction-question-score">درجة الطالب في السؤال<input type="number" data-awarded-index="${i}" data-question-max="${safe(points)}" min="0" max="${safe(points)}" step="0.25" value="${safe(awarded)}" oninput="recalculateCorrectionScore()"><span>من ${safe(points)}</span></label></div>`;}).join('')||'<p class="section-desc">لا توجد إجابات محفوظة.</p>'}</div><div class="correction-final-v40"><button class="small-btn" type="button" onclick="recalculateCorrectionScore()">إعادة حساب المجموع</button><label>الدرجة النهائية من ${safe(max)}<input id="manualFinalScore" type="number" min="0" max="${safe(max)}" step="0.25" value="${safe(a.score??a.autoScore??0)}" readonly></label><button class="btn primary" onclick="saveAttemptCorrection('${safe(a.id)}')">حفظ التصحيح والنتيجة</button></div></div></div>`;document.body.insertAdjacentHTML('beforeend',html);recalculateCorrectionScore();};
window.closeCorrectionModal=function(){document.querySelector('.correction-modal-v40')?.remove();};
window.recalculateCorrectionScore=function(){const fields=[...document.querySelectorAll('.correction-modal-v40 [data-awarded-index]')],input=document.getElementById('manualFinalScore');if(!fields.length||!input)return;input.value=Math.round(fields.reduce((sum,field)=>sum+Math.max(0,Math.min(Number(field.dataset.questionMax)||0,Number(field.value)||0)),0)*100)/100;};
window.saveAttemptCorrection=async function(id){const attempt=adminData.examAttempts.find(item=>String(item.id)===String(id));if(!attempt)return;const before=JSON.parse(JSON.stringify(attempt)),fields=[...document.querySelectorAll('.correction-modal-v40 [data-awarded-index]')];let correct=0;for(const field of fields){const index=Number(field.dataset.awardedIndex),max=Number(field.dataset.questionMax)||0,awarded=Number(field.value);if(!Number.isFinite(awarded)||awarded<0||awarded>max)return aToast(`درجة السؤال ${index+1} يجب أن تكون من 0 إلى ${max}`);if(attempt.answers?.[index]){attempt.answers[index].awardedScore=awarded;attempt.answers[index].correct=awarded===max;attempt.answers[index].teacherReviewed=true;}if(awarded===max)correct++;}const maxScore=Number(attempt.maxScore||fields.reduce((sum,field)=>sum+(Number(field.dataset.questionMax)||0),0)||100),score=Number(document.getElementById('manualFinalScore')?.value);if(Number.isNaN(score)||score<0||score>maxScore)return aToast(`الدرجة النهائية يجب أن تكون من 0 إلى ${maxScore}`);attempt.score=score;attempt.maxScore=maxScore;attempt.percentage=maxScore?Math.round(score/maxScore*100):0;attempt.correctCount=correct;attempt.questionCount=fields.length||attempt.questionCount||0;attempt.needsManualReview=false;attempt.status='corrected';attempt.teacherCorrectedAt=new Date().toISOString();try{if(!window.MFCloud?.saveExamAttempt)throw new Error('Exam correction service unavailable');await window.MFCloud.saveExamAttempt(attempt);saveData(adminData);aToast(`تم حفظ النتيجة: ${score} من ${maxScore}`);closeCorrectionModal();renderExams();}catch(error){Object.keys(attempt).forEach(key=>delete attempt[key]);Object.assign(attempt,before);saveData(adminData);aToast(adminActionErrorMessage(error,'تعذر حفظ التصحيح.'));}};
...
function examBuilderTotal(root=document){return Math.round([...root.querySelectorAll('[data-exam-question]')].reduce((sum,card)=>sum+(Number(card.querySelector('[data-question-points]')?.value)||0),0)*100)/100;}
function ensureExamScoreControls(form){
  if(!form||form.dataset.scoreControlsReady==='1')return;
  form.dataset.scoreControlsReady='1';
  const duration=form.elements.duration?.closest('.field'),grid=duration?.parentElement;
  if(grid&&!form.elements.maxScore)duration.insertAdjacentHTML('afterend','<div class="field exam-max-score-field"><label>الدرجة النهائية للامتحان</label><input name="maxScore" type="number" min="0.25" step="0.25" value="1" required><small>يجب أن تساوي مجموع درجات الأسئلة.</small></div>');
  const list=form.querySelector('.exam-questions-builder');
  const syncHint=()=>{const total=examBuilderTotal(list||form),hint=form.querySelector('.exam-max-score-field small');if(hint)hint.textContent=`مجموع درجات الأسئلة الآن: ${total}`;};
  list?.addEventListener('input',syncHint);syncHint();
  form.addEventListener('submit',event=>{if(form.elements.sourceMode?.value==='pdf')return;const total=examBuilderTotal(list||form),max=Number(form.elements.maxScore?.value);if(!Number.isFinite(max)||max<=0||Math.abs(total-max)>.001){event.preventDefault();event.stopImmediatePropagation();aToast(`الدرجة النهائية لازم تساوي مجموع درجات الأسئلة (${total})`);}},true);
}
function serializeExamQuestions(root=document){const cards=[...root.querySelectorAll('[data-exam-question]')];return cards.map(card=>{const question=card.querySelector('[data-question-text]')?.value.trim(),type=card.querySelector('[data-question-type]')?.value||'mcq',points=Number(card.querySelector('[data-question-points]')?.value);if(!question||!Number.isFinite(points)||points<=0)return null;const mark=`الدرجة: ${points}`;if(type==='essay')return `${question}\n${mark}`;if(type==='truefalse'){const answer=card.querySelector('[data-truefalse-answer]')?.value||'أ';return `${question}\nأ) صح\nب) غلط\nالإجابة: ${answer}\n${mark}`;}const options=[...card.querySelectorAll('[data-question-option]')].map(input=>input.value.trim()),answer=card.querySelector('[data-correct-answer]')?.value;if(options.some(value=>!value)||!answer)return null;return `${question}\nأ) ${options[0]}\nب) ${options[1]}\nج) ${options[2]}\nد) ${options[3]}\nالإجابة: ${answer}\n${mark}`;}).filter(Boolean).join('\n\n');}
...
function academicExamRow(item){
  const state=examClientAvailability(item,Date.now()),open=examTimeMs(item.openAt)||0,close=examTimeMs(item.closeAt)||0,target=state==='upcoming'?open:state==='open'?close:0,label=state==='upcoming'?'يفتح بعد':state==='open'?(close?'يغلق بعد':'متاح الآن'):'انتهى الوقت',badgeClass=state==='open'?'good':state==='upcoming'?'warn':'danger';
  return `<div class="mobile-row academic-content-row" data-academic-countdown-target="${target}" data-academic-countdown-kind="${state==='upcoming'?'opening':state==='open'?'closing':'closed'}"><div><b>${safe(item.title)}</b><small>${safe(item.grade)} · ${safe(item.group||'كل المجموعات')} · ${safe(item.duration||20)} دقيقة</small><small>${item.openAt?'يفتح: '+safe(formatExamCairoDateTime(item.openAt)):'متاح فورًا'}${item.closeAt?' · يغلق: '+safe(formatExamCairoDateTime(item.closeAt)):''}</small></div><span class="badge ${badgeClass}">${label} <b data-academic-countdown>${target?'--:--:--':state==='open'?'بدون موعد إغلاق':'انتهى'}</b></span>${item.pdfUrl?`<a class="small-btn" href="${safe(item.pdfUrl)}" target="_blank">PDF</a>`:''}<button class="small-btn danger" onclick="deleteItem('exams','${safe(item.id)}')">حذف</button></div>`;
}
function startAcademicAdminCountdowns(){
  clearInterval(window.__academicCountdownTimer);
  window.__academicCountdownRefreshPending=false;
  const tick=()=>document.querySelectorAll('[data-academic-countdown-target]').forEach(row=>{const target=Number(row.dataset.academicCountdownTarget||0),out=row.querySelector('[data-academic-countdown]');if(!target||!out||row.dataset.countdownExpired==='1')return;const left=Math.max(0,target-Date.now()),total=Math.ceil(left/1000),days=Math.floor(total/86400),hours=Math.floor(total%86400/3600),minutes=Math.floor(total%3600/60),seconds=total%60;out.textContent=days?`${days}ي ${hours}س`:`${String(hours).padStart(2,'0')}:${String(minutes).padStart(2,'0')}:${String(seconds).padStart(2,'0')}`;if(!left){row.dataset.countdownExpired='1';out.textContent=row.dataset.academicCountdownKind==='opening'?'متاح الآن':'انتهى';if(!window.__academicCountdownRefreshPending){window.__academicCountdownRefreshPending=true;setTimeout(()=>{window.__academicCountdownRefreshPending=false;if(currentSection==='academics')renderAcademics();else if(currentSection==='exams')renderExams();},1200);}}});
...
async function saveUnifiedAcademicExam(event){
  ...
  if(form.openAt.value&&form.closeAt.value&&examTimeMs(form.closeAt.value)<=examTimeMs(form.openAt.value))return aToast('موعد الإغلاق يجب أن يكون بعد موعد الفتح');
  if(file&&(file.type!=='application/pdf'||file.size>15*1024*1024))return aToast('اختار PDF صحيحًا بحجم لا يزيد عن 15MB');
  const parsed=parseExamQuestions(text),values=Object.fromEntries(new FormData(form).entries());delete values.pdfFile;delete values.sourceMode;values.id=`ex-${Date.now()}`;values.grade=grade;values.group=group;values.text=text;values.pdfOnly=pdfOnly;values.duration=Math.max(1,Math.min(240,Number(values.duration||20)));values.openAt=examCairoIso(values.openAt);values.closeAt=examCairoIso(values.closeAt);values.allowRetake=form.allowRetake.checked;values.active=true;values.questionCount=parsed.length;values.mcqCount=parsed.filter(q=>q.type==='mcq').length;values.essayCount=parsed.filter(q=>q.type==='essay').length;values.createdAt=new Date().toISOString();
...
async function saveUnifiedAcademicAssignment(event){
  event.preventDefault();const form=event.currentTarget,button=form.querySelector('[type="submit"]'),grade=document.getElementById('academicComposerGrade')?.value,group=document.getElementById('academicComposerGroup')?.value||'كل المجموعات',file=form.elements.file?.files?.[0]||null,list=form.querySelector('#unifiedAssignmentQuestions'),text=serializeExamQuestions(list),cards=[...list.querySelectorAll('[data-exam-question]')],startedCards=cards.filter(card=>String(card.querySelector('[data-question-text]')?.value||'').trim()),count=startedCards.length;
  if(!String(form.elements.title?.value||'').trim())return aToast('اكتب عنوان الواجب');
  if(!file&&!count)return aToast('أضف سؤالًا واحدًا على الأقل أو ارفع ملف الواجب');
  if(count&&(!text||text.split('\n\n').length!==count))return aToast('كمّل كل سؤال بدأت كتابته وحدد إجابته الصحيحة');
  if(file&&file.size>15*1024*1024)return aToast('حجم الملف أكبر من 15MB');
  const values=Object.fromEntries(new FormData(form).entries());delete values.file;values.id=`as-${Date.now()}`;values.grade=grade;values.group=group;values.academicYear=adminData.settings?.academicYear||'';values.questions=text?text.split(/\n\s*\n/).map(value=>value.replace(/\n(?:الإجابة|الاجابة)\s*:[^\n]*$/i,'').trim()).filter(Boolean):[];values.questionText=text;values.questionCount=values.questions.length;values.publishAt=values.publishAt?new Date(values.publishAt).toISOString():new Date().toISOString();values.active=true;values.createdAt=new Date().toISOString();
