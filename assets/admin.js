...
function bookingCard(b){return `<article class="booking-admin-card"><div><span class="badge warn">ينتظر القبول</span><h3>${safe(b.name||b.studentName)}</h3><small>${safe(b.code)} · ${safe(b.grade)} · ${safe(b.group)}</small></div><div class="booking-admin-details"><span><small>رقم الطالب</small><b dir="ltr">${safe(b.studentPhone||'-')}</b></span><span><small>ولي الأمر</small><b dir="ltr">${safe(b.parentPhone||'-')}</b></span><span><small>الشهر</small><b>${safe(b.month||'-')}</b></span></div><div class="mobile-actions"><button class="btn primary" type="button" onclick="approveBooking('${safe(b.code)}')"><span data-icon="user-check"></span> قبول وتسجيل الطالب</button><button class="btn danger" type="button" onclick="deleteBooking('${safe(b.code)}')">رفض وحذف</button></div></article>`;}
window.approveBooking=async function(code){
  ...
  }catch(error){aToast(error?.message?.split(':').pop()?.trim()||'تعذر قبول الحجز. تأكد من نشر Functions والاتصال بالإنترنت.');return false;}
};
