...
window.openQrScanner=async function(){const modal=document.getElementById('qrScannerModal');if(!modal)return;modal.hidden=false;qrAttendanceProcessing=false;qrAttendanceLastValue='';qrAttendanceLastTime=0;qrAttendanceSessionCount=0;const reader=document.getElementById('adminQrReader');if(!reader)return;reader.insertAdjacentHTML('beforebegin','<div id="continuousQrStatus" class="continuous-qr-status ready"><b>جاهز لمسح الطالب الأول</b><small>سيظل الماسح مفتوحًا لتسجيل الطلاب واحدًا وراء الآخر</small></div>');reader.innerHTML='<p class="section-desc">جاري تجهيز الكاميرا…</p>';try{if(!window.Html5Qrcode)await window.MFAssets?.loadQrScanner?.();reader.innerHTML='';if(window.Html5Qrcode){qrScanner=new Html5Qrcode('adminQrReader');await qrScanner.start({facingMode:'environment'},{fps:10,qrbox:{width:250,height:250}},decoded=>{handleContinuousQrScan(decoded);});}else if('BarcodeDetector'in window){reader.innerHTML='<video id="adminQrVideo" autoplay playsinline></video>';const video=document.getElementById('adminQrVideo');const stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:'environment'}});video.srcObject=stream;const detector=new BarcodeDetector({formats:['qr_code']});const loop=async()=>{if(document.getElementById('qrScannerModal')?.hidden)return;const codes=await detector.detect(video).catch(()=>[]);if(codes.length)await handleContinuousQrScan(codes[0].rawValue);setTimeout(loop,500);};loop();}else{reader.innerHTML='<p class="section-desc">المتصفح لا يدعم ماسح QR. استخدم إدخال كود يدوي.</p>';}}catch(e){reader.innerHTML='<p class="section-desc">تعذر فتح الكاميرا. تأكد من السماح للمتصفح باستخدام الكاميرا.</p>';updateContinuousQrStatus('تعذر تشغيل الكاميرا','error');}};
