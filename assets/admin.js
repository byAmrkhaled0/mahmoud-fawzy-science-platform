  ...
  root.innerHTML=`
    ...
    <main class="admin-main">
      <header class="admin-mobile-header">
        ...
        <div><small>لوحة التحكم</small><strong id="adminCurrentSectionLabel">${safe(adminSectionName(currentSection))}</strong></div>
      ...
      <div class="admin-top">
        <div><span class="kicker"><span data-icon="sparkles"></span> Admin Dashboard</span><h1 class="section-title">لوحة تحكم مستر محمود إبراهيم فوزي</h1></div>
        <div class="header-actions"><button class="theme-toggle" id="themeToggleAdmin" aria-label="تغيير الوضع"></button><button class="btn ghost" type="button" onclick="forceFirestoreSync()"><span data-icon="refresh-cw"></span><span>حفظ ومزامنة</span></button><button class="btn dark" type="button" onclick="adminLogout()">خروج</button></div>
      </div>
      <div id="adminContent"></div>
    </main>
    <nav class="admin-mobile-bottom" aria-label="التنقل السريع في لوحة التحكم">
      ${[['overview','bar-chart','الرئيسية'],['students','users','الطلاب'],['attendance','qr','الحضور'],['exams','clipboard','الامتحانات']].map(([id,ic,name])=>`<button type="button" data-admin-nav="${id}" class="${id===currentSection?'active':''}"><span data-icon="${ic}"></span><span>${name}</span></button>`).join('')}
...
window.deleteBooking=function(code){if(!confirm('حذف الحجز؟'))return; adminData.bookings=adminData.bookings.filter(b=>b.code!==code); persist('تم حذف الحجز'); deferAdminRender(renderBookings); Promise.all([cloudDelete('bookings',code),cloudDelete('booking_status',code)]).catch(()=>{});};
