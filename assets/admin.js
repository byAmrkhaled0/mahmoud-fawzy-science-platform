...
window.toggleExamQuestionType=function(select){const card=select.closest('[data-exam-question]'),isEssay=select.value==='essay',mcq=card?.querySelector('[data-mcq-fields]'),note=card?.querySelector('[data-essay-note]');if(mcq)mcq.hidden=isEssay;if(note)note.hidden=!isEssay;card?.querySelectorAll('[data-question-option],[data-correct-answer]').forEach(input=>{input.required=!isEssay;});};
function serializeExamQuestions(){const cards=[...document.querySelectorAll('[data-exam-question]')];return cards.map(card=>{const question=card.querySelector('[data-question-text]')?.value.trim(),type=card.querySelector('[data-question-type]')?.value||'mcq';if(!question)return null;if(type==='essay')return question;const options=[...card.querySelectorAll('[data-question-option]')].map(input=>input.value.trim()),answer=card.querySelector('[data-correct-answer]')?.value;if(options.some(value=>!value)||!answer)return null;return `${question}\nأ) ${options[0]}\nب) ${options[1]}\nج) ${options[2]}\nد) ${options[3]}\nالإجابة: ${answer}`;}).filter(Boolean).join('\n\n');}
