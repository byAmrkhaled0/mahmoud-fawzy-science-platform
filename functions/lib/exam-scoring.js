...
function assignQuestionScores(questions, configuredMaxScore) {
  ...
  if (!explicit) {
    ...
    rows.forEach((question, index) => {
      question.points = index === rows.length - 1
        ? Math.round((desiredTotal - assignedTotal) * 100) / 100
        : Math.round(share * 100) / 100;
      assignedTotal += question.points;
    });
  } else {
    rows.forEach(question => { question.points = positiveScore(question.points, 1); });
  }
  return rows;
}
...
function scoreSummary(questions, awardedScores, pendingManual = false) {
  ...
  return {
    score: pendingManual ? null : awarded,
    maxScore,
    percentage: pendingManual || !maxScore ? null : Math.round(awarded / maxScore * 100)
  };
}
