...
const syncSource = fs.readFileSync(path.join(root, 'assets/firebase-sync.js'), 'utf8');
if (!syncSource.includes('createBookingDirect') || !syncSource.includes("collection('booking_status')")) {
  fail('Booking reliability fallback is missing');
}
if (!failures.some(x => x.includes('Booking reliability'))) ok('Booking callable and Firestore fallback checks passed');
...
if (!rules.includes('match /booking_status/{bookingCode}') || !rules.includes('allow get: if validCode(bookingCode);')) {
  fail('Booking status fallback rules are missing');
}
